...
const ShowPage = () => {
  ...
  const [showData, setShowData] = useState({});
  const [showCast, setShowCast] = useState([]); // Correction : tableau vide par défaut
  const [seasonCount, setSeasonCount] = useState(0);
  const [liked, setLiked] = useState(false);

  useEffect(() => {
    const fetchById = async () => {
      const apiUrlId = `https://api.tvmaze.com/shows/${id}?embed[]=episodes&embed[]=seasons&embed[]=cast`; // Ajout de cast
      ...
      try {
        ...
        setShowData(data);
        setSeasonCount(data._embedded.seasons.length);
        setShowCast(data._embedded.cast); // Mise à jour de showCast
      } catch (error) {
        console.log('Error fetching show data', error);
      }
    };
  ...
  const handleLikeToggle = async () => {
    if (!liked) {
      ...
      const bookmarkData = {
        ...
        status: showData.status,
        seasonCount: seasonCount,
        bookmarkedEpisodes: {}, // Pour les épisodes favoris futurs
      };
  ...
  // Récupérer jusqu'à 5 acteurs pour affichage
  const cast = showCast.slice(0, 5).map((castMember) => castMember.person.name);
  ...
  const formattedCast = cast.length > 0 ? cast.join(', ') : 'Cast not available';
